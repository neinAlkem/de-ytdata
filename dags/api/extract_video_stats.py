...
@task
def get_video_ids(playlist_id: str, max_results: int = MAX_RESULTS, api_key: str = API_KEY) -> list:
    """Get all video ids from playlist i
    Args:
...
def save_stats_to_json(stats: list, filename: str) -> None:
    ...
    """
    filepath = f'data/{filename}_{date.today()}.json'
    try:
